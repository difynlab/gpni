...
@section('content')
    ...
    <div class="static-pages">
        <form action="{{ route('backend.purchases.final-exam-purchases.refund-status', $final_exam_purchase->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="section">
                    ...
                    <div class="col-6">
                        ...
                        <div style="background: #fffbea; border: 1px solid #f0c040; border-radius: 8px; padding: 12px;">
                            ...
                            <select class="form-control" name="refund_status">
                                <option value="Not Refunded" {{ $final_exam_purchase->refund_status != 'Refunded' ? 'selected' : '' }}>Not Refunded</option>
                                <option value="Refunded" {{ $final_exam_purchase->refund_status == 'Refunded' ? 'selected' : '' }}>Refunded</option>
                            </select>
                        </div>
                    </div>
                </div>
