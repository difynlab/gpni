...
@section('title', 'Insurance & Professional Membership With IICT')

@push('after-styles')
    <link rel="stylesheet" href="{{ asset('frontend/css/insurance.css') }}">
@endpush
...
@section('content')
    ...
    <div class="container bg-white section my-5">
        <h2 class="title-main">{{ $contents->{'section_1_title_' . $language} ?? $contents->{'section_1_title_en'} }}</h2>
        <div class="d-flex-custom">
            <div>
                @if($contents->{'section_1_image_' . $language})
                    <img src="{{ asset('storage/backend/pages/' . $contents->{'section_1_image_' . $language}) }}" alt="Cover image" class="img-fluid img-fluid-custom"
    ...
    <!-- Section 2 -->
    @if($contents->{'section_2_content_en'})<!-- section_2_title_en NOT ADDED IN THE DATABASE TABLE. COLUMN [section_2_content_en] USED IN HERE INSTEAD. -->
    <div class="container-fluid section approve-section approve-section-background">
        <div class="container">
            <div class="d-flex-custom">
                <div>
                    <h2 class="title-white">Approved by IICT</h2><!-- section_2_title_en NOT ADDED IN THE DATABASE TABLE.-->
                    <div class="content-text-white">
                    <!-- data added in the database as <ul> followed by <li> -->
                        {!! $contents->{'section_2_content_' . $language} ?? $contents->{'section_2_content_en'} !!}
                    </div>
                </div>
                <div class="flex-shrink-0">
                    @if($contents->{'section_2_image_' . $language})
                        <img src="{{ asset('storage/backend/pages/' . $contents->{'section_2_image_' . $language}) }}" alt="Certified" class="img-fluid img-fluid-custom"
    ...
    <!-- Section 4 -->
    @if($contents->{'section_4_content_en'})<!-- section_4_title_en NOT ADDED IN THE DATABASE TABLE. COLUMN [section_4_content_en] USED IN HERE INSTEAD. -->
        ...
        <div class="container">
            <div class="d-flex-custom align-items-center">
                <div class="text-center">
                    @if($contents->{'section_4_image_' . $language})
                        <img src="{{ asset('storage/backend/pages/' . $contents->{'section_4_image_' . $language}) }}" alt="Importance of insurance"
                ...
                <div style="max-width: 726px;">
                    <h2 class="title-white">Importance of insurance</h2><!-- section_4_title_en NOT ADDED IN THE DATABASE TABLE.-->
                    <div class="content-text-white">
                    <!-- data added in the database as <ul> followed by <li> -->
            ...
            <div class="highlight-box">
                <p class="highlight-text-bold">
                    {!! strip_tags(explode('</p>', $contents->{'section_4_sub_content_' . $language} ?? $contents->{'section_4_sub_content_en'})[0]) !!}
                </p>
                <p class="highlight-text-regular">
                    {!! strip_tags(explode('</p>', $contents->{'section_4_sub_content_' . $language} ?? $contents->{'section_4_sub_content_en'})[1]) !!}
                </p>
            </div>
        </div>
    </div>
