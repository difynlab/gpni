...
@section('content') 
    ...
    @if($contents->title_en)
        <div class="container py-5">
            <div class="heading text-center">{{ $contents->{'title_' . $middleware_language} ?? $contents->title_en }}</div>
            <div class="pt-0">
                <x-frontend.notification></x-frontend.notification>
                <x-frontend.notification-popup></x-frontend.notification-popup>
            </div>
            <p class="description sub-heading text-center">{{ $contents->{'sub_title_' . $middleware_language} ?? $contents->sub_title_en }}</p>

            <p class="description sub-heading text-center">{{ $contents->{'description_' . $middleware_language} ?? $contents->description_en }}</p>
                ...
                <div class="col-lg-6 col-md-12">
                    <form action="{{ route('frontend.gift-cards.checkout') }}" method="POST" onsubmit="return validateForm()">
                        ...
                        <div class="form-group pb-3">
                            <label for="receiver-name" class="text-heading pb-1">{{ $contents->{'receiver_name_' . $middleware_language} ?? $contents->receiver_name_en }}</label>
                            <input type="text" class="form-control text-content" id="receiver-name" name="receiver_name" required>
                        </div>

                        <div class="form-group pb-3">
                            <label for="receiver-email" class="required text-heading pb-1">{{ $contents->{'receiver_email_' . $middleware_language} ?? $contents->receiver_email_en }}</label>
                            <input type="email" class="form-control" id="receiver-email" name="receiver_email" required>
                        </div>

                        <div class="form-group pb-3">
                            <label class="required text-heading pb-1">{{ $contents->{'select_amount_' . $middleware_language} ?? $contents->select_amount_en }}</label>
                        ...
                        <button type="submit" class="blue-button btn-responsive">{{ $contents->{'button_' . $middleware_language} ?? $contents->button_en }}</button>
                    </form>
                </div>
            </div>
        </div>
