...
class GiftCardPurchaseController extends Controller
{
    private function processGiftCardPurchases($gift_card_purchases)
    {
        foreach($gift_card_purchases as $gift_card_purchase) {
            ...
            $gift_card_purchase->refund_status_badge =
                ($gift_card_purchase->refund_status == 'Refunded')
    ...
    public function updateRefundStatus(Request $request, GiftCardPurchase $gift_card_purchase)
    {
        $request->validate([
            'refund_status' => 'required|in:Refunded,Not Refunded',
        ]);

        $gift_card_purchase->refund_status = $request->refund_status;
        $gift_card_purchase->save();

        return redirect()->route('backend.purchases.gift-card-purchases.index')->with('success', 'Refund status updated successfully!');
    }
