...
class UserController extends Controller
{
    public function index(Request $request)
    {
        if(session('users')) {
            ...
            $users = User::where('status', '!=', '0')->orderBy('id', 'desc')->paginate($items);
            $name = null;
            $email = null;
            $role = null;
        }
        ...
        return view('backend.administrations.users', [
            'users' => $users,
            'name' => $name,
            'email' => $email,
            'role' => $role,
    ...
    public function update(Request $request, User $user)
    {
        ...
        return redirect()->route('backend.users.index')->with('success', "Successfully updated!");
    }
