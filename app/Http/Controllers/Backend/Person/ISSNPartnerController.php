...
class ISSNPartnerController extends Controller
{
    private function processISSNPartners($issn_partners)
    {
        foreach($issn_partners as $issn_partner) {
            ...
            $issn_partner->image = $issn_partner->image != null ? '<img src="'. asset('storage/backend/persons/issn-partners/' . $issn_partner->image) .'" class="table-image">' : '<img src="'. asset('storage/common/' . Setting::find(1)->no_image) .'" class="table-image">';
