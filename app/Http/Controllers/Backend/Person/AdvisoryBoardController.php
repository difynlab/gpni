...
class AdvisoryBoardController extends Controller
{
    private function processAdvisoryBoards($advisory_boards)
    {
        foreach($advisory_boards as $advisory_board) {
            ...
            $advisory_board->image = $advisory_board->image != null ? '<img src="'. asset('storage/backend/persons/advisory-boards/' . $advisory_board->image) .'" class="table-image">' : '<img src="'. asset('storage/common/' . Setting::find(1)->no_image) .'" class="table-image">';
